...
#if defined(__AVR_ATmega328P__) //DMD To be sure that the right board is selected
  #define BOOTTIME         2000 //DMD Ori 2000   // Time in milliseconds that we show boot loading bar and wait user input
#endif
...
void loop() 
{
  ...
  if( osd_state_screen & MASK_OSD_GET_RSSI_CONF ) {
    mavlink_msg_param_request_read_send( MAVLINK_COMM_0, apm_mav_system, apm_mav_component, RSSI_PARAM_NAME, -1);
  } 
