...
void writePanels(){ 
    ...
    if(millis() < (lastMAVBeat + 2000)){
      ...
      if (osd_set == 0) { // setup panel is called in the else at the end
        if (panel != npanels) // npanels is always 1 more than the number of panels, so the last panel is the osd off panel
        {
          ...
          if(ISa(panel, Bp_BIT)) panBattery_A_Percent(panBattery_A_Percent_XY[0][panel], panBattery_A_Percent_XY[1][panel]); //
          ...
          //  if(ISb(panel, WDir_BIT)) panWayPDir(panWayPDir_XY[0][panel], panWayPDir_XY[1][panel]); //??x??
          //  if(ISb(panel, WDis_BIT)) panWayPDis(panWayPDis_XY[0][panel], panWayPDis_XY[1][panel]); //??x??
          if(ISb(panel, MavB_BIT)) panMavBeat(panMavBeat_XY[0][panel], panMavBeat_XY[1][panel]); //13x3
          ...
          if(ISc(panel, Vel_BIT)) panGndSpd(panVel_XY[0][panel], panVel_XY[1][panel]); //
          ...
          if(ISd(panel,Warn_BIT)) panWarn(panWarn_XY[0][panel], panWarn_XY[1][panel]); // this must be here so warnings overwrite OSD
        }
        else
        {
          if(ISd(0, Warn_BIT)) panWarn(panWarn_XY[0][0], panWarn_XY[1][0]); // this must be here so warnings overwrite OSD
        }
      } else { // if (osd_set == 1)
        doSetup();
      }
...
void panLogo(){
  ...
  osd.openPanel();
  osd.printf_P(PSTR("\x20\x20\x20\x20\x20\xba\xbb\xbc\xbd\xbe|\x20\x20\x20\x20\x20\xca\xcb\xcc\xcd\xce|MinimOSD Alter|1.30.1 Beta"));
  osd.closePanel();
}
...
void panWaitMAVBeats(int first_col, int first_line){
    ...
    osd.setPanel(first_col, first_line);
    osd.openPanel();
    osd.printf_P(PSTR("Waiting for|MAVLink data..."));
    osd.closePanel();
}
...
void panOff(){
  ...
  if (osd_toggle_chan == 0) {
    if (((apm_mav_type == 1) && ((osd_mode != 11) && (osd_mode != 10))) || ((apm_mav_type == 2) && ((osd_mode != 3) && (osd_mode != 6)))){
    ...
    if (osd_toggle_chan_raw > 1700 && millis() - osd_switch_time > 1000)
    {
      osd_switch_time = millis();
      incPanel();
    }
...
void incPanel()
{
  ...
  if (millis() <= 60000 && panel == npanels) // npanels is the osd off panel
    osd_set = 1;
...
void doSetup(){
  ...
  if (millis() > text_timer){
    ...
    if ((chan2_raw - 100) > chan2_raw_middle) {
      ...
      if (setup_menu > 7) setup_menu = 0;
    }
      
    if ((chan2_raw + 100) < chan2_raw_middle) {
      setup_menu = setup_menu - 1;
      if (setup_menu < 0) setup_menu = 7;
    }
    ...
    case 0:
      {
          ...
          if (EEPROM.read(measure_ADDR) == 0)
              osd.printf_P(PSTR("Metric"));
          else
              osd.printf_P(PSTR("Imperial"));
        break;
      }
    case 1:
    {
        if ((chan1_raw - 100) > chan1_raw_middle){
          osd_toggle_temp++;
          if (osd_toggle_temp > 4) osd_toggle_temp = 0;
        }
        if ((chan1_raw + 100) < chan1_raw_middle){
          osd_toggle_temp--;
          if (osd_toggle_temp < 0) osd_toggle_temp = 4;
        }
        osd.printf_P(PSTR("Panel change switch"));
    ...
    case 2:
      {
        ...
        osd.printf("%3.0i%c", overspeed, speedPic);
        break;
      }
    case 3:
      {
        ...
        osd.printf("%3.0i%c", stall , speedPic);
        break;
      }
    case 4:
      {
        stall = change_val(overalt, overalt_ADDR);
        osd.printf_P(PSTR("Height Limit"));
        osd.printf("%3.0i%c", overalt, heightPic);
        break;
      }
    case 5:
      {
        battv = change_val(battv, battv_ADDR);
        osd.printf_P(PSTR("Low Battery Volts"));
        osd.printf("%3.1f%c", float(battv)/10.0, 0x76);
        break;
      }
        //      case 4:
        //        battv = change_val(battp, battp_ADDR);
        //        osd.printf_P(PSTR("Battery Percent"));
        //        osd.printf("%3.0i%c", battp , 0x25);

        //        break;
     case 6:
     {
        user_rssi_high = change_val(user_rssi_high, OSD_HIGH_ADDR);
        osd.printf_P(PSTR("Set RSSI 100% value"));
        osd.printf_P(PSTR("|Tx on ="));
        osd.printf("%3.0i", mav_rssi);
        osd.printf_P(PSTR("|EEPROM value ="));
        osd.printf("%3.0i", user_rssi_high);
        break;
     }
     case 7:
     {
        user_rssi_low = change_val(user_rssi_low, OSD_LOW_ADDR);
        osd.printf_P(PSTR("Set RSSI 0% value"));
        osd.printf_P(PSTR("|Turn Tx off ="));
        osd.printf("%3.0i", mav_rssi);
        osd.printf_P(PSTR("|EEPROM value ="));
        osd.printf("%3.0i", user_rssi_low);
        break;
      }
    }
  }
...
void panWarn(int first_col, int first_line){
    ...
    osd.openPanel();
      ...
      if (millis() > text_timer){ // if the text has been shown for a while
          ...
          while (warning_type == 0) { // cycle through the warning checks
            ...
            switch(x) {
              case 1:
                ...
                break;
              case 2:
                if (osd_airspeed * converts < stall && osd_airspeed > 1.39) warning_type = 2; // 1.39 m/s = 5 kph (roughly)
                break;
              case 3:
              ...
              case 4:
                ...
                break;
              case 5:
                if (osd_battery_remaining_A < battp) warning_type = 4;
                break;
              case 6:
                if ((osd_alt - osd_home_alt) > overalt) warning_type = 5;
              break;
            }
        ...
        if (warning_type > 0 && osd_toggle_chan == 0) panel = 1; // turn OSD on if there is a warning and we're using flight modes to toggle OSD
        ...
        switch(warning_type){
          ...
          case 1: // No GPS Fix!
            ...
            break;
          case 2: // Stall!
            osd.printf_P(PSTR("\x20\x20\x20\x53\x74\x61\x6c\x6c\x21"));
            break;
          case 3: // Overspeed!
            osd.printf_P(PSTR("\x20\x4f\x76\x65\x72\x53\x70\x65\x65\x64\x21"));
            break;
          case 4: // Battery Low!
            osd.printf_P(PSTR("\x42\x61\x74\x74\x65\x72\x79\x20\x4c\x6f\x77\x21"));
            break;
          case 5: // Too high!
            osd.printf_P(PSTR("\x20\x54\x6F\x6F\x20\x48\x69\x67\x68\x21"));
            break;
          }
       }
...
void panWindSpeed(int first_col, int first_line){
    ...
    osd_wind_arrow_rotate_int = round((osd_winddirection - osd_yaw)/360.0 * 16.0); //Convert to int 0-16 
    if(osd_wind_arrow_rotate_int < 0 ) osd_wind_arrow_rotate_int += 16; //normalize
    osd.printf("%c%3.0f%c|",0xFC,(double)(osd_windspeed * converts), speedPic);
    showArrow(osd_wind_arrow_rotate_int);
...
void panRSSI(int first_col, int first_line){
  osd.setPanel(first_col, first_line);
  osd.openPanel();
  osd_rssi = (mav_rssi - user_rssi_low) * 99 / (user_rssi_high - user_rssi_low);
  osd.printf("%c%2i%c", 0xE1, osd_rssi, 0x25); 
  osd.closePanel();
}
...
void panBattery_A_Percent(int first_col, int first_line){
    osd.setPanel(first_col, first_line);
    osd.openPanel();
    osd.printf("%c%4.0i%c%c", 0x61, (int)osd_batt_mahused_A, 0x8D, 0x8F); // setBatteryPic(osd_battery_remaining_A)
    osd.closePanel();
}
...
void panBatt_A(int first_col, int first_line){
    ...
    */
    osd.printf("%c%5.1f%c", 0xE2, (double)osd_vbat_A, 0x8E);
    osd.closePanel();
}
...
void panCur_A(int first_col, int first_line){
    osd.setPanel(first_col, first_line);
    osd.openPanel();
    osd.printf("%c%5.1f%c", 0xE4, (osd_curr_A * .01), 0x8F);
    osd.closePanel();
}
...
void panAlt(int first_col, int first_line){
    ...
    osd.printf("%c%5.0f%c",0xE6, (double)(osd_alt * converth), heightPic);
    osd.closePanel();
}
...
void panHomeAlt(int first_col, int first_line){
    ...
    osd.openPanel();
    //osd.printf("%c%5.0f%c",0x85, (double)(osd_alt - osd_home_alt), 0x8D);
    osd.printf("%c%5.0f%c",0xE7, (double)((osd_alt - osd_home_alt) * converth), heightPic);
    osd.closePanel();
}
...
void panGndSpd(int first_col, int first_line){
    osd.setPanel(first_col, first_line);
    osd.openPanel();
    osd.printf("%c%3.0f%c",0xE9,(double)(osd_groundspeed * converts), speedPic);
    osd.closePanel();
}
...
void panAirSpeed(int first_col, int first_line){
    osd.setPanel(first_col, first_line);
    osd.openPanel();
    osd.printf("%c%3.0f%c", 0xE8, (double)(osd_airspeed * converts), speedPic);
    osd.closePanel();
}
...
void panHomeDis(int first_col, int first_line){
    osd.setPanel(first_col, first_line);
    osd.openPanel();
    osd.printf("%c%5.0f%c", 0x1F, (double)((osd_home_distance) * converth), heightPic);
    osd.closePanel();
}
...
//    osd.printf("%11.6f%c%c%c%c%c%c%c%c%11.6f", (double)osd_lat, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, (double)osd_lon);
    osd.closePanel();
}
...
void panHeading(int first_col, int first_line){
    osd.setPanel(first_col, first_line);
    osd.openPanel();
    osd.printf("%4.0f%c", (double)osd_gps_heading, 0xb0);
    osd.closePanel();
}
...
//    osd.printf("%s|%c5i%s%c", "\x20\xc0\xc0\xc0\xc0", osd_yaw, "\xc0\xc0\xc0\xc0\x20", 0xd0, buf_show, 0xd1);
...
//    osd.setPanel(first_col, first_line);
//    osd.openPanel();
//    showArrow( need waypoint direction );
//    osd.closePanel();
//}
