...
void GPS_NewData() {
  ...
  if (GPS_active>0)
    GPS_active--;
  if (msgbuf.nmea.fix && (msgbuf.nmea.sats >= MINSATFIX)) {
    if (msgbuf.nmea.fix_HOME == 0){
      GPS_reset_home_position();
      msgbuf.nmea.fix_HOME=1;
      if (!msgbuf.nmea.fix_HOME) {
    ...
    uint32_t dist;
    int32_t  dir;
    GPS_distance_cm_bearing(&msgbuf.nmea.lat,&msgbuf.nmea.lon,&GPS_home[LAT],&GPS_home[LON],&dist,&dir);
    GPS_distanceToHome = dist/100;
    GPS_directionToHome = dir/100;
    msgbuf.nmea.alt =  msgbuf.nmea.alt- msgbuf.nmea.alt_home;
    MwAltitude = msgbuf.nmea.alt *100;
    GPS_latitude = msgbuf.nmea.lat;
    GPS_longitude = msgbuf.nmea.lon;
    int16_t MwHeading360=msgbuf.nmea.course/10;
    if (MwHeading360>180)
    ...
    if (GPS_armedangleset==1){
      if ((GPS_distanceToHome<20)&&(msgbuf.nmea.speed<75)){
...
void GPS_reset_home_position() {
  if (msgbuf.nmea.fix && msgbuf.nmea.sats >= MINSATFIX) {
      GPS_home[LAT] = msgbuf.nmea.lat;
      GPS_home[LON] = msgbuf.nmea.lon;
      msgbuf.nmea.alt_home = msgbuf.nmea.alt;
      GPS_calc_longitude_scaling(msgbuf.nmea.lat);  //need an initial value for distance and bearing calc
      msgbuf.nmea.fix_HOME = 1;
  }
}
